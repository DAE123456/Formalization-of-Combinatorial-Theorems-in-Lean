...
-/
lemma exsist_walk_inter_eq_singlon {G : SimpleGraph V} [Finite V] [DecidableEq V]
    {hG : G.Connected} {a b : V} (p : G.Walk a b) (hp : Walk.IsMaxlongPath p) :
    ∀ (v : V), ∃ (j : ℕ), G.Reachable v (p.getVert j) ∧
...
lemma in_subset_eq_in_all_1 {G : SimpleGraph V} [DecidableEq V] {a b x : V} (p : G.Walk a b)
    {i : ℕ} {hi : p.getVert i ∈ p.reverse.support} {h : x ∈ (p.reverse.takeUntil _ hi).support}
     : x ∈ p.support := by
    ...
    · rw [← hk]
      ...
      · have l : p.getVert i ∈ p.reverse.support := by
          simp only [support_reverse, mem_reverse, getVert_mem_support]
        have len : (p.reverse.takeUntil _ hi).length ≤ p.reverse.length := by
          apply SimpleGraph.Walk.length_takeUntil_le
        omega
    · exact hj


lemma in_subset_eq_in_all_2 {G : SimpleGraph V} [DecidableEq V] {a b x : V} (p : G.Walk a b)
    {i j : ℕ} {hi : p.getVert i ∈ p.support} {hj : p.getVert (j + 1) ∈ (p.takeUntil _ hi).reverse.support}
    {h : x ∈ ((p.takeUntil _ hi).reverse.takeUntil _ hj).support}
     : x ∈ p.support := by
    have : ((p.takeUntil _ hi).reverse.takeUntil _ hj).support ⊆ (p.takeUntil _ hi).reverse.support := by
      apply SimpleGraph.Walk.support_takeUntil_subset
    simp only [support_reverse, subset_reverse] at this
    rw [List.subset_def] at this
    obtain h := this h
    have l : (p.takeUntil _ hi).support ⊆ p.support := by
      apply SimpleGraph.Walk.support_takeUntil_subset
...
lemma in_subset_eq_in_all_3 {G : SimpleGraph V} [DecidableEq V] {a b x : V} (p : G.Walk a b)
    {i : ℕ} {hi : p.getVert i ∈ p.support} {h : x ∈ (p.takeUntil _ hi).support}
     : x ∈ p.support := by
    rw [SimpleGraph.Walk.mem_support_iff_exists_getVert] at h
    rcases h with ⟨k , hk, hj⟩
    rw [SimpleGraph.Walk.getVert_takeUntil] at hk
    · rw [← hk]
      ...
      · have l : p.getVert i ∈ p.reverse.support := by
          simp only [support_reverse, mem_reverse, getVert_mem_support]
        have len : (p.takeUntil _ hi).length ≤ p.length := by
          apply SimpleGraph.Walk.length_takeUntil_le
        omega
    · exact hj
...
lemma tail_Disjoint_1 {G : SimpleGraph V} [DecidableEq V] {a b u v : V} (p : G.Walk a b)
    (s : G.Walk v u) {hs : s.IsPath} {h_inter : s.support ∩ p.support = {u}}
    {i : ℕ} {hi : p.getVert i ∈ p.support} {hu : u = p.getVert i} :
    (p.takeUntil _ hi).support.Disjoint s.reverse.support.tail := by
  rw [← List.inter_eq_nil_iff_disjoint, ← List.toFinset_eq_empty_iff]
  simp only [support_reverse, tail_reverse, inter_reverse, toFinset_inter]
  apply Finset.Subset.antisymm
  · intro x hx
    rcases Finset.mem_inter.mp hx with ⟨hx_left, hx_right⟩
    have h1 : p.getVert i ∈ p.support := by simp only [getVert_mem_support]
    simp only [mem_toFinset] at hx_left
    simp only [mem_toFinset] at hx_right
    have h_indices_left : x ∈ p.support := by
      obtain h := in_subset_eq_in_all_3 (p := p) (i := i) (hi := by simp) (h := hx_left)
      exact h
    have h_indices_right : ¬ x ∈ p.support := by
      ...
      have h'' : s.support ≠ [] := by
        simp_all
      have s_eq : (s.support.dropLast ++ [p.getVert i]).reverse= s.support.reverse := by
        rw [← List.dropLast_append_getLast (l := s.support)]
        · simp only [getLast_support, ne_eq, cons_ne_self, not_false_eq_true,
          dropLast_append_of_ne_nil, dropLast_singleton, List.append_nil, List.reverse_append,
          List.reverse_cons, List.reverse_nil, List.nil_append, List.cons_append, List.cons.injEq,
          and_true]
          exact hu.symm
        · exact h''
...
-/
lemma len_reverse_takeUntil {G : SimpleGraph V} [DecidableEq V] {a b : V} (p : G.Walk a b)
 {hp : p.IsPath} {i : ℕ} {hi : p.getVert i ∈ p.reverse.support} :
...
lemma getVert_congr {G : SimpleGraph V}
    ...
    {hn1 : i ≤ p.length} {hn2 : j ≤ p.length} : i = j := by
...
lemma next_not_exsit_takeUntil {G : SimpleGraph V} [DecidableEq V] {u v : V} (p : G.Walk u v)
  {hp : p.IsPath} {j : ℕ} (h : p.getVert j ∈ p.support) (len : j < p.length) :
...
lemma h_indices_trans_4 {G : SimpleGraph V} [DecidableEq V] {a b x : V} (p : G.Walk a b) {hp : p.IsPath} {i : ℕ}
  {hi : p.getVert i ∈ p.support} {hx : x ∈ (p.dropUntil _ hi).support.tail}
  {len : i ≤ p.length} {h_adj : G.Adj (p.getVert i) b} :
  ∃ j, p.getVert j = x ∧ j ≥ i + 1 ∧ j ≤ p.length:= by
  rw [← SimpleGraph.Walk.support_tail_of_not_nil] at hx
  · rw [SimpleGraph.Walk.mem_support_iff_exists_getVert ] at hx
    rcases hx with ⟨j , hj, hj_len⟩
    rw [SimpleGraph.Walk.getVert_tail] at hj
    have : (p.dropUntil _ hi).tail.length = (p.dropUntil _ hi).length - 1 := by
      nth_rw 2 [← SimpleGraph.Walk.length_tail_add_one]
      · simp only [add_tsub_cancel_right]
      · apply SimpleGraph.Walk.not_nil_of_ne
        by_contra H
        simp only [H, SimpleGraph.irrefl] at h_adj
    rw [this] at hj_len
    have len : (p.takeUntil _ hi).length = i := by
      rw [len_takeUntil (hp := hp) (hn := by omega)]
    use (i + j + 1)
    have hj_in : p.getVert (i + j + 1) ∈ p.support := by
    ...
    rw [getVert_dropUntil hi len.symm, ← Nat.add_assoc] at hj
    · refine ⟨hj, ?_⟩
      · have : (p.dropUntil _ hi).length = p.length - ((p.takeUntil _ hi).length) := by
          have len : p.length = (p.dropUntil _ hi).length + (p.takeUntil _ hi).length := by
            rw [add_comm, ← SimpleGraph.Walk.length_edges, ← SimpleGraph.Walk.length_edges,← SimpleGraph.Walk.length_edges, ← List.length_append]
            conv_rhs => enter [1]; rw [← SimpleGraph.Walk.edges_append (p := p.takeUntil _ hi), SimpleGraph.Walk.take_spec]
          omega
        rw [this, len] at hj_len
        rw [Nat.sub_sub, le_tsub_iff_left, Nat.add_assoc, Nat.add_comm (n := 1), ← Nat.add_assoc] at hj_len
        · omega
...
lemma Disjoint_tail_2 {G : SimpleGraph V} [DecidableEq V] {a b u v : V} (p : G.Walk a b)
    ...
    {hu : u = p.getVert j} :
    s.support.Disjoint ((p.takeUntil _ hj).reverse.takeUntil _ hi).support.tail := by
  rw [← List.inter_eq_nil_iff_disjoint, ← List.toFinset_eq_empty_iff]
  simp only [toFinset_inter]
  apply Finset.Subset.antisymm
  · intro x hx
...
lemma tail_Disjoint_copy_3 {G : SimpleGraph V} {i : ℕ} [DecidableEq V] {a b u v : V} (p : G.Walk a b)
    (s : G.Walk v (p.getVert i)) (s' : G.Walk v u) {hs : s.IsPath} {h_inter : s.support ∩ p.support = {p.getVert i}}
    {hu : p.getVert i = u} (h_copy : s' = s.copy rfl hu) : p.support.Disjoint s'.reverse.support.tail := by
  rw [← List.inter_eq_nil_iff_disjoint, ← List.toFinset_eq_empty_iff]
  simp only [support_reverse, tail_reverse, inter_reverse, toFinset_inter]
  apply Finset.Subset.antisymm
  · intro x hx
    ...
    have h_indices_right : ¬ x ∈ p.support := by
      ...
      obtain h := List.mem_of_mem_dropLast hx_right
      obtain h' := List.mem_inter_of_mem_of_mem h h_indices_left
      obtain h_copy' := SimpleGraph.Walk.support_copy s rfl hu
      rw [← h_copy'] at h_inter
      rw [h_copy] at h'
      rw [h_inter,  List.singleton_eq, List.mem_singleton] at h'
      have h'' : s'.support ≠ [] := by
        simp_all
      have s_eq : (s'.support.dropLast ++ [p.getVert i]).reverse =
...
lemma tail_Disjoint_4 {G : SimpleGraph V} [DecidableEq V] {a b v : V} (p : G.Walk a b)
      {i j : ℕ} {hj : p.getVert j ∈ p.support} {hi_succ : p.getVert (i + 1) ∈ p.reverse.support}
      (s : G.Walk v (p.getVert j)) {hp : p.IsPath} {hs : s.IsPath} {h_inter : s.support ∩ p.support = {(p.getVert j)}}
      {h_len : i < p.length} {hij : (i + 1) > j} :
      (p.reverse.takeUntil _ hi_succ).support.Disjoint ((p.takeUntil _ hj).append s.reverse).support := by
  have h_disjoint : (p.takeUntil _ hj).support.Disjoint s.reverse.support.tail := by
    obtain H := tail_Disjoint_1 (p := p) (s := s) (hs := hs) (h_inter := h_inter) (i := j) (hi := hj) (hu := rfl)
    exact H
  rw [← List.inter_eq_nil_iff_disjoint]
  rw [SimpleGraph.Walk.support_append]
  obtain h_perm := List.Perm.inter_append h_disjoint (l := (p.reverse.takeUntil _ hi_succ).support)
  obtain h := List.toFinset_eq_of_perm (h := h_perm)
  rw [← List.toFinset_eq_empty_iff, h]
  simp_all only [support_reverse, tail_reverse, disjoint_reverse_right,
    inter_reverse, toFinset_inter, toFinset_append, toFinset_reverse, union_eq_empty]
  constructor
  · apply Finset.Subset.antisymm
    · intro x hx
      rcases Finset.mem_inter.mp hx with ⟨hx_left, hx_right⟩
      ...
      have h_indices_left : ∃ k, p.reverse.getVert k = x ∧ k ≤ p.length - (i + 1) := by
        obtain H := h_indices_trans_1 (hp := hp) (hx := hx_left) (i := i + 1) (h := hi_succ) (len := by omega)
        apply H
      have h_indices_right : ∃ k, p.getVert k = x ∧ k ≤ j := by
        obtain H := h_indices_trans_2 (hp := hp) (hx := hx_right) (i := j) (h := hj) (len := by omega)
        apply H
      ...
      have hk_eq : k₁ = p.length - k₂ := by
        obtain congr := getVert_congr (h := hk₁_eq) (hp := hp) (hn1 := by omega) (hn2 := by omega)
  ...
  · apply Finset.Subset.antisymm
    · intro x hx
      ...
      have h2 : x ∈ p.support := by
        obtain h := in_subset_eq_in_all_1 (p := p) (i := i + 1) (hi := hi_succ) (h := hx_left)
        exact h
      have h3 : x = p.getVert j := by
        have h_inter' : x ∈ s.support ∩ p.support := by
          simp only [mem_inter_iff]
          refine ⟨h1, h2⟩
        rw [h_inter, List.singleton_eq] at h_inter'
...
-/
lemma ore_endpoints_adjacent {G : SimpleGraph V} [Finite V] (hG : G.Connected) {a b : V}
 {p : G.Walk a b} (hp : Walk.IsMaxlongPath p) (hv_not_in_p : ∃ (v : V), v ∉ p.support) :
  ¬ G.Adj a b := by
  have : Fintype V := Fintype.ofFinite V
  ...
  have H : ∃ j : ℕ, G.Reachable v (p.getVert (j)) ∧
      ∃ s : G.Walk v (p.getVert (j)), s.IsPath ∧ s.support ∩ p.support = {p.getVert (j)} := by
    obtain H := exsist_walk_inter_eq_singlon (hG := hG) (p := p) hp v
    exact H
  ...
  · have h1 : p.getVert j ∈ p.support := by simp only [getVert_mem_support]
    ...
    have h2' : p.getVert (j + 1) ∈ p.reverse.support := by simp only [support_reverse, mem_reverse, getVert_mem_support]
    ...
    have hq_path : q.IsPath := by
      unfold q
      rw [SimpleGraph.Walk.isPath_def]
      obtain H := List.nodup_append' (l₁ :=  (p.reverse.takeUntil _ h2').support.reverse) (l₂ := ((p.takeUntil _ h1).append s.reverse).support)
      ...
      · simp only [nodup_reverse]
        ...
        exact hp_path
      · constructor
        · obtain H := List.nodup_append' (l₁ :=(p.takeUntil _ h1).support) (l₂ := s.reverse.support.tail)
          have : ((p.takeUntil _ h1).append s.reverse).support = ((p.takeUntil _ h1).support ++ s.reverse.support.tail) := by
            rw [Walk.support_append]
          rw [this, H]
          constructor
          · rw [← SimpleGraph.Walk.isPath_def]
          ...
          · constructor
            · rw [← SimpleGraph.Walk.support_tail_of_not_nil]
              ...
              · intro H
                ...
                rw [← H'] at h1
                contradiction
            · have H3 : (p.takeUntil _ h1).support.Disjoint s.reverse.support.tail := by
                obtain H := tail_Disjoint_1 (p := p) (s := s) (hs := hs_path) (h_inter := hs_inter) (i := j) (hi := h1) (hu := rfl)
                exact H
              exact H3
        · have H4 : (p.reverse.takeUntil (p.getVert (j + 1)) h2').support.reverse.Disjoint
            ...
            simp only [disjoint_reverse_left]
            obtain H := tail_Disjoint_4 (p := p) (s := s) (hp := hp_path) (h_inter := hs_inter) (i := j) (j := j) (hi_succ := h2') (h_len := by omega) (hij := by omega) (hs := hs_path)
            exact H
          exact H4
    have hq_length : q.length = s.length + p.length := by
      ...
      obtain congr := getVert_congr (h := H) (hp := hp_path) (hn1 := by apply SimpleGraph.Walk.length_takeUntil_le) (hn2 := by omega)
      rw [congr]
      obtain H := SimpleGraph.Walk.getVert_length_takeUntil h2'
      rw [SimpleGraph.Walk.getVert_reverse] at H
      obtain congr := getVert_congr (h := H) (hp := hp_path) (hn1 := by omega) (hn2 := by omega)
      have : p.length - (j + 1) = (p.reverse.takeUntil (p.getVert (j + 1)) h2').length := by omega
      rw [← this, add_right_comm, ← Nat.add_assoc, Nat.sub_add_cancel]
      · omega
      · omega
    ...
    have hq_path : q.IsPath := by
      ...
      · constructor
        ...
        · have H3 : p.support.Disjoint s'.reverse.support.tail := by
            obtain H := tail_Disjoint_copy_3 (p := p) (s := s) (s' := s') (hs := hs_path) (h_inter := hs_inter) (hu := eq) (h_copy := by simp [s'])
            exact H
          exact H3
...
-/
lemma endpoint_ne [Fintype V] {G : SimpleGraph V} {hG : G.Connected} {a b : V} (p : G.Walk a b)
 (hp : Walk.IsMaxlongPath p) {h_order : Fintype.card V ≥ 3} (hv_not_in_p : ∃ (v : V), v ∉ p.support) :
  a ≠ b := by
  unfold Walk.IsMaxlongPath at hp
  obtain ⟨hp_path, hp_max⟩ := hp
  rcases hv_not_in_p with ⟨v, hv_not_in_p⟩
...
lemma exsist_neighbor_first_point {G : SimpleGraph V} {a b v : V} (p : G.Walk a b)
  ...
  have hv_mem : v ∈ p.support := by
    by_contra H
    simp only [G.mem_neighborSet] at h_neighbor_a
...
lemma exsist_neighbor_end_point {G : SimpleGraph V} {a b v : V} (p : G.Walk a b)
  ...
  have hv_mem : v ∈ p.support := by
    by_contra H
    simp only [G.mem_neighborSet] at h_neighbor_b
    ...
    have hq_path : q.IsPath := by
      ...
      have : (p.append (Walk.cons hav nil)).support.Nodup = (p.support ++ ((Walk.cons hav nil)).support.tail).Nodup  := by
        rw [Walk.support_append]
      rw [SimpleGraph.Walk.isPath_def]
      rw [this, H]
    ...
    · let I : Finset (Fin (p.support.length - 1)) :=
      ...
      have disjoint : I ∩ J = ∅ := by
        ...
        have h_y_eq : y = p.getVert (i.val) := by
          ...
          have := i.2
          omega
        obtain H := exsist_walk_inter_eq_singlon (hG := hG) (p := p) (hp := ⟨hp_path, hp_max⟩) v
            ...
            · have H'' : (p.takeUntil (p.getVert j) h1).support.Disjoint s.reverse.support.tail := by
              ...
              have H3 : eady_xb.reverse.support.Disjoint (Walk.cons h_adj_ax.symm (s.append eady_ja).reverse).support.tail := by
                have l_nodup : (eady_xb.reverse.support ++ (Walk.cons h_adj_ax.symm (s.append eady_ja).reverse).support.tail).Nodup := by
                  ...
                  have : ((p.takeUntil _ h1).append s.reverse).support.Nodup = ((p.takeUntil _ h1).support ++ s.reverse.support.tail).Nodup  := by
                    rw [Walk.support_append]
                  ...
                  · constructor
                    · rw [this, List.nodup_append']
                      ...
                      · rw [← SimpleGraph.Walk.isPath_def]
                        ...
                        exact hp_path
                      · constructor
                        · have : ¬ s.reverse.Nil := by
                            simp only [nil_reverse]
                            by_contra! H
                            obtain l := SimpleGraph.Walk.Nil.eq H
                            simp_all only [not_true_eq_false]
                          obtain h := tail_Ispath (p := s.reverse) (hp := by simp only [isPath_reverse_iff]; omega) (h := this)
                          exact h
                        · exact H''
                    · have h4 : p.getVert (j + 1) ∈ p.reverse.support := by simp only [support_reverse, mem_reverse, getVert_mem_support]
                      obtain H := tail_Disjoint_4 (p := p) (s := s) (hp := hp_path) (hs := hs_path) (h_inter := hs_inter) (i := i) (j := j) (hi_succ := h2) (h_len := by omega) (hij := by omega)
                      apply H
                simp_all only [ge_iff_le, ne_eq, support_reverse, support_cons, List.tail_cons,
              ...
              · have h_jsucc : p.getVert (j + 1) ∈ (p.takeUntil _ h3).reverse.support := by
                ...
                · apply Finset.Subset.antisymm
                  · intro x hx
                    ...
                    simp only [mem_toFinset] at hx_left
                    have h_indices_left : ∃ k, p.getVert (i - k) = x ∧ k ≤ i - (j + 1)  := by
                      obtain H := h_indices_trans_3 (hp := hp_path) (hx := hx_left) (i := i) (j := j + 1) (hn := h_jsucc) (len := by omega) (hl := by omega)
                      exact H
                    ...
                    · by_contra H
                      have hx_left : x ∈ p.support := by
                        obtain h := in_subset_eq_in_all_2 (p := p) (i := i) (hi := h3) (h := hx_left) (hj := by omega)
                        exact h
            ...
            · have eq : p.getVert i = p.getVert j := by simp only [hI]
              ...
              have hq_path : q.IsPath := by
                ...
                have H_x_not_in : x ∉ (p.takeUntil _ h_in_i).support := by
                    simp [h_x_eq]
                    have : ¬ p.getVert (i + 1) ∈ (p.takeUntil _ h3).support := by
                ...
                · apply tail_Ispath
                  ...
                  · simp only [eady_isucc_b]
                    ...
                    simp_all only [ge_iff_le, ne_eq, length_support, add_tsub_cancel_right,
                      ...
                      not_false_eq_true, implies_true, eq_iff_iff, forall_const]
                    rw [← hI] at l
                  ...
                  · intro x hx
                    ...
                    · have h : (p.takeUntil _ h_in_i).support.tail ⊆ (p.takeUntil _ h_in_i).support := by
                      ...
                      have h3 : x = p.getVert j := by
                        ...
                        simp_all only [ge_iff_le, ne_eq, not_le,
                          ...
                          have h_indices_right : ∃ k, p.getVert k = x ∧ k ≥ (j + 1) ∧ k < p.length := by
                            ...
                            have not_nil : ¬ eady_bj_succ.Nil := by
                              simp_all only [ge_iff_le, ne_eq, length_support,
                                ...
                                getVert_mem_support, implies_true, eq_iff_iff, mem_inter,
      ...
      have card_I : I.card = G.degree a := by
        ...
        have h1 : ({i | G.Adj a (p.support.get ⟨(i + 1) , by omega⟩)} :
          ...
          ext i; simp only [get_eq_getElem, Finset.mem_filter, mem_univ, true_and, I]
        have h2 : ({x | x ∈ {x | G.Adj a x}} )= (G.neighborSet a).toFinset := by
          ext v; simp only [Set.mem_setOf_eq, Set.coe_toFinset, mem_neighborSet]
        ...
        have hsurj : Function.Surjective f := by
          ...
          have : ∃ (i : Fin (p.support.length - 1)), p.support.get ⟨i.val + 1, by omega⟩ = v := by
            obtain H := exsist_neighbor_first_point (hp := ⟨hp_path, hp_max⟩) (a := a) (b := b) (v := v) (h_neighbor_a := hv)
            exact H
          rcases this with ⟨i, hi⟩
          have mem : i ∈ I := by
            unfold I
            simp only [List.get_eq_getElem, Finset.mem_filter, mem_univ, true_and]
      ...
      have card_J : J.card = G.degree b := by
        ...
        have h1 : ({i | G.Adj (p.support.get ⟨i, by omega⟩) b} :
          ...
          ext i; simp only [get_eq_getElem, Finset.mem_filter, mem_univ, true_and, J]
        ...
        have hsurj : Function.Surjective f := by
          ...
          have : ∃ (i : Fin (p.support.length - 1)), p.support.get ⟨i.val, by omega⟩ = v := by
            obtain H := exsist_neighbor_end_point (hp := ⟨hp_path, hp_max⟩) (a := a) (b := b) (v := v) (h_neighbor_b := hv)
            exact H
          rcases this with ⟨i, hi⟩
          have mem : i ∈ J := by
            unfold J
            ...
            exact hv.symm
          refine ⟨⟨i, mem⟩, ?_⟩
          simp_all only [get_eq_getElem, List.get_eq_getElem, f]
        exact Equiv.ofBijective f ⟨hinj, hsurj⟩
      ...
      have h_union_bound : I.card + J.card ≤ p.length := by
        calc
        I.card + J.card = (I ∪ J).card := by omega
        _ ≤ p.support.length - 1 := by
          simp only [length_support, add_tsub_cancel_right]
          have h_union_bound : (I ∪ J).card ≤ p.support.length - 1 := by
...
-/
lemma maximal_hamiltonian {G : SimpleGraph V} [Fintype V] [G.LocallyFinite] [DecidableEq V]
   {hG : G.Connected} {a b : V} (p : G.Walk a b) (hp : Walk.IsMaxlongPath p) {h_order : Fintype.card V ≥ 3}
     {h_ore : ∀ u v : V, u ≠ v → ¬ G.Adj u v → G.degree u + G.degree v ≥ Fintype.card V} :
    p.IsHamiltonian := by
  classical
...
lemma ore_adjacent {G : SimpleGraph V} (hG : G.Connected) [Fintype V] [G.LocallyFinite]
  (h_ore : ∀ u v : V, u ≠ v → ¬ G.Adj u v → G.degree u + G.degree v ≥ Fintype.card V)
  {h_order : Fintype.card V ≥ 3} {a b : V} {h_ne : a ≠ b} {p : G.Walk a b} {hp : Walk.IsMaxlongPath p} {h_ab : ¬ G.Adj a b}:
  ∃ (k : Fin (p.support.length - 1)), G.Adj a (p.getVert (k.val + 1)) ∧ G.Adj  (p.getVert k.val) b := by
  ...
  classical
  let F : V → Fin (p.support.length) := fun v =>
  ...
  have h_card_I : I.card = G.degree a := by
    ...
    have hsurj : Function.Surjective f := by
      intro ⟨v, hv⟩
      have : ∃ (i : Fin (p.support.length - 1)), p.support.get ⟨i.val + 1, by omega⟩ = v := by
        obtain H := exsist_neighbor_first_point (hp := hp) (a := a) (b := b) (v := v) (h_neighbor_a := hv)
        exact H
      rcases this with ⟨i, hi⟩
      have mem : i ∈ I := by
        unfold I
  ...
  have h_card_J : J.card = G.degree b := by
    ...
    have hsurj : Function.Surjective f := by
      ...
      simp only [G.mem_neighborSet] at hv
      have : ∃ (i : Fin (p.support.length - 1)), p.support.get ⟨i.val, by omega⟩ = v := by
        obtain H := exsist_neighbor_end_point (hp := hp) (a := a) (b := b) (v := v) (h_neighbor_b := hv)
        exact H
      rcases this with ⟨i, hi⟩
      have mem : i ∈ J := by
        unfold J
  ...
  have h_disjoint : ¬ Disjoint I J := by
    ...
    have h_union_bound : I.card + J.card ≤ p.length := by
      ...
      _ = p.length := by
        simp
    rw [h_total_size] at h_union_bound
    have h_total_lower_bound : Fintype.card V ≥ p.length + 1 := by
  ...
  use k
  let x := p.support.get ⟨k.val , by omega⟩
  let y := p.support.get ⟨k.val + 1, by omega⟩
...
lemma tail_Disjoint_5 {G : SimpleGraph V} [DecidableEq V] {a b : V} (p : G.Walk a b)
  {i : ℕ} {hi : p.getVert i ∈ p.support} {hi_succ : p.getVert (i + 1) ∈ p.reverse.support}
  {hp : p.IsPath} {h_len : i < p.length} :
  (p.reverse.takeUntil _ hi_succ).support.Disjoint (p.takeUntil _ hi).support := by
  rw [← List.inter_eq_nil_iff_disjoint, ← List.toFinset_eq_empty_iff]
  simp only [toFinset_inter]
  apply Finset.Subset.antisymm
  · intro x hx
    ...
    simp only [mem_toFinset] at hx_left
    simp only [mem_toFinset] at hx_right
    have h_indices_left : ∃ j , p.reverse.getVert j = x ∧ j ≤ p.length - (i + 1) := by
      obtain H := h_indices_trans_1 (hp := hp) (hx := hx_left) (i := i + 1) (h := hi_succ) (len := by omega)
      apply H
    · have h_indices_right : ∃ j, p.getVert j = x ∧ j ≤ i := by
        obtain H := h_indices_trans_2 (hp := hp) (hx := hx_right) (i := i) (h := hi) (len := by omega)
        apply H
      rcases h_indices_left with ⟨k₁, hk₁_eq, hk₁_ge⟩
      rcases h_indices_right with ⟨k₂, hk₂_eq, hk₂_le⟩
      rw [SimpleGraph.Walk.getVert_reverse] at hk₁_eq
...
theorem Ore_theorem {G : SimpleGraph V} [Fintype V] [DecidableEq V] [G.LocallyFinite]
  ...
  have len : p.length = p.support.length - 1 := by
    rw [SimpleGraph.Walk.length_support]
    omega
  ...
  have h_path_len : p.length = Fintype.card V - 1 := by
    classical
    rw [SimpleGraph.Walk.IsHamiltonian.length_eq]
    apply maximal_hamiltonian (hG := hG) (hp := h_maxpath) (h_order := h_order) (h_ore := h_ore)
  ...
  · let cycle := p.concat h_adj.symm
    ...
    have cycle_tail_support_Nodup : cycle.tail.support.Nodup := by
    ...
    · rw [SimpleGraph.Walk.isCycle_def]
      ...
      · rw [SimpleGraph.Walk.isTrail_def]
        ...
        · by_contra! h_edge_in
          ...
          · have li : i  < p.edges.length  := by omega
            obtain H := Walk.edge_get_verts (p := p) (hp := h_maxpath.1) i li
            by_cases hi0 : i.1= 0
            · simp_all only [ge_iff_le, get_eq_getElem, Sym2.eq, Sym2.rel_iff', Prod.mk.injEq,
              and_false, Prod.swap_prod_mk, or_true, getVert_zero, zero_add, and_true]
              rcases H with (h1| h3)
              · simp_all only [not_true_eq_false]
              · have l: p.getVert p.length = b := by simp only [getVert_length]
                conv at l => right; rw [h3]
                obtain congr := getVert_congr (h := l) (hp := h_maxpath.1) (hn1 := by omega) (hn2 := by simp [SimpleGraph.Walk.length_edges] at li; omega)
                omega
            obtain H := Walk.edge_get_verts p h_maxpath.1 i li
            rw [hi] at H
            simp only [Sym2.eq, Sym2.rel_iff', Prod.mk.injEq, Prod.swap_prod_mk] at H
            have : a = p.getVert 0  := by simp
            rcases H with (h1 | h2)
            · obtain h1' := h1.2.symm
              conv at li => right; rw [SimpleGraph.Walk.length_edges]
              obtain l1 := SimpleGraph.Walk.IsPath.getVert_eq_start_iff (p := p) (hp := h_maxpath.1) (i := (i + 1)) li
              rw [h1'] at l1
              simp only [Nat.add_eq_zero_iff, one_ne_zero, and_false, iff_false,
                not_true_eq_false] at l1
            · obtain h1'  := h2.2
              conv at h1' => left; rw [this]
              obtain congr := getVert_congr (h := h1') (hp := h_maxpath.1) (hn1 := by omega) (hn2 := by simp [SimpleGraph.Walk.length_edges] at li; omega)
              omega
      · constructor
        · intro h
          unfold cycle at h
          ...
          omega
        · rw [← SimpleGraph.Walk.support_tail_of_not_nil]
          · exact cycle_tail_support_Nodup
          · by_contra! h
            rw [SimpleGraph.Walk.nil_iff_support_eq] at h
            simp only [cycle] at h
            simp_all only [ge_iff_le, ne_eq, support_concat, List.concat_eq_append,
              append_left_eq_self, support_ne_nil]
    · intro v
  ...
  · have h_ne : a ≠ b := by omega
    ...
    let eaxy_ak : G.Walk a (p.getVert k) := p.takeUntil _ hk
    ...
    have H5 : (eaxy_ksucc_b.append (Walk.cons h1.symm eaxy_ak)).support.Nodup := by
      ...
      · constructor
        ...
        · simp only [support_cons, List.tail_cons, eaxy_ksucc_b, eaxy_ak]
          have H4 : (p.reverse.takeUntil _ hk').support.Disjoint ((p.takeUntil _ hk)).support := by
            obtain H := tail_Disjoint_5 (p := p) (i := k) (hi := hk) (hi_succ := hk') (hp := h_maxpath.1) (h_len := by omega)
            exact H
          exact H4
    constructor
    · rw [SimpleGraph.Walk.isCycle_def]
      ...
      · rw [SimpleGraph.Walk.isTrail_def]
        ...
        · constructor
          · simp only [eaxy_ksucc_b]
            ...
            rw [SimpleGraph.Walk.mem_support_iff_exists_getVert] at H
            rcases H with ⟨i, hi, h_len⟩
            conv at hi => right; rw [show k = p.length - (p.length - k) by omega]; rw [← SimpleGraph.Walk.getVert_reverse]
            rw [SimpleGraph.Walk.getVert_takeUntil] at hi
            · have : p.reverse.IsPath := by
                simp only [isPath_reverse_iff]
                exact h_maxpath.1
              have l : (p.reverse.takeUntil (p.getVert (↑k + 1)) hk').length ≤ p.reverse.length := by
                apply SimpleGraph.Walk.length_takeUntil_le
              obtain congr := getVert_congr (h := hi) (hp := this) (hn1 := by omega) (hn2 := by simp)
              · have len : (p.reverse.takeUntil (p.getVert (↑k + 1)) hk').length = p.length - (k + 1) := by
                  rw [len_reverse_takeUntil (p := p) (hp := h_maxpath.1) (i := k + 1) (hi := hk')]
                rw [len] at h_len
                omega
            · exact h_len
          · constructor
            ...
            · simp only [eaxy_ak]
              by_contra h_edge_in
              obtain H := SimpleGraph.Walk.snd_mem_support_of_mem_edges (p := (p.takeUntil _ hk)) h_edge_in
              rw [SimpleGraph.Walk.mem_support_iff_exists_getVert] at H
              rcases H with ⟨n, h1, h2⟩
              rw [SimpleGraph.Walk.getVert_takeUntil] at h1
              · rw [SimpleGraph.Walk.IsPath.getVert_eq_end_iff] at h1
                · have len_take_k : (p.takeUntil _ hk).length = k := by
                    apply Eq.symm
                  ...
                  omega
                · exact h_maxpath.1
                · obtain hl := SimpleGraph.Walk.length_takeUntil_le p hk
                  omega
              · exact h2
        · rw [← SimpleGraph.Walk.edges_cons (h := by rw [adj_comm] at h1; exact h1), ← SimpleGraph.Walk.edges_append]
          apply SimpleGraph.Walk.edges_nodup_of_support_nodup
          exact H5
      · constructor
        · unfold cycle
    ...
    · unfold cycle
      ...
      · intro v
        ...
        have hv_subset : p.support ⊆ ((Walk.cons h2 eaxy_ksucc_b).append (Walk.cons h1.symm eaxy_ak)).tail.support := by
          ...
          rw [← SimpleGraph.Walk.concat_append, SimpleGraph.Walk.support_append, SimpleGraph.Walk.support_concat]
          simp only [List.concat_eq_append, List.append_assoc, List.cons_append, List.nil_append]
          rw [← SimpleGraph.Walk.support_eq_cons]
          have hp_split : p.support = ((p.takeUntil _ hk).append (p.dropUntil _ hk)).support := by
            obtain H := SimpleGraph.Walk.take_spec p hk
            conv_lhs => rw [← H]
          conv_lhs => rw [hp_split]
          rw [← List.append_eq_has_append, SimpleGraph.Walk.support_append, ← List.append_eq_has_append]
          simp only [append_eq, append_subset, List.Subset.refl, subset_append_of_subset_right, true_and]
          apply List.subset_append_of_subset_left
          rw [List.subset_def]
          intro v hv
          have h_indices_hv: ∃ j, p.getVert j = v ∧ j ≥ k + 1 ∧ j ≤ p.length:= by
            obtain H := h_indices_trans_4 (hp := h_maxpath.1) (hx := hv) (i := k) (hi := hk) (len := by omega) (h_adj := h2)
            exact H
          rw [SimpleGraph.Walk.mem_support_iff_exists_getVert]
          rcases h_indices_hv with ⟨j, h1, h2⟩
          use (p.length - j)
          have len : (p.reverse.takeUntil _ hk').length = p.length - (k + 1) := by
            rw [len_reverse_takeUntil (p := p) (hp := h_maxpath.1) (i := k + 1) (hi := hk')]
          constructor
          · rw [SimpleGraph.Walk.getVert_takeUntil]
